...
type CheckType<T, U = unknown> = Defined<T> extends Obj & {
    withComponent: Func;
} ? '🅂🄲' : Defined<T> extends string ? '🅂' : Defined<T> extends number ? '🄽' : Defined<T> extends boolean ? '🄱' : Defined<T> extends Func & {
    withComponent: Func;
} ? '🅂🄲' : Defined<T> extends {
    name: string;
    styles: string;
    map?: string;
} ? '🄲' : Defined<T> extends Obj ? '🄾' : Defined<T> extends Func ? '🄵' : Defined<T> extends Array<U> ? '🄰' : Defined<T> extends ReadonlyArray<U> ? '🄰' : '�';
